...
export const FlightForm = ({
  ...
  const handleFormSubmit = (values) => {
    ...
    if (flight?.id) {
      ...
      updateTransportation(flight.id, payload)
        .then((result) => {
          if (files.length > 0) {
      ...
      <form onSubmit={form.onSubmit((values) => handleFormSubmit(values))}>
        <Stack>
          ...
          <Group>
            <TextInput
              name={'airline'}
              label={t('transportation.airline', 'Airline')}
              required
              key={form.key('airline')}
              {...form.getInputProps('airline')}
            />
            <TextInput
              name={'flightNumber'}
              label={t('transportation.flight_number', 'Flight Number')}
              required
                ...
                <Title size={'md'}>
                  ...
                  <Text size={'xs'} c={'dimmed'}>
                    {t(
                      ...
                      'Upload any related documents for this flight e.g. confirmation email'
                    )}
                  </Text>
                </Title>
              </Group>
                    ...
                    if (flight?.id) {
                      return (
                        <Stack>
                          <Text
                            size={'xs'}
                          >{`${flight.attachments ? flight.attachments.length : 0} existing files`}</Text>
                          <Button {...props}>
