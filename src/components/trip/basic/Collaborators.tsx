...
const UserSearch = ({trip, setNewCollaborators}: {
  trip: Trip,
...
export const Collaborators = ({context, id, innerProps}: ContextModalProps<{
  trip: Trip,
  onSave: () => void
}>) => {
    ...
    <Container>
      ...
      <Group mt={"md"}>
        {(trip.collaborators || []).map((collaborator,) => {
          return (<Group wrap={"nowrap"} key={collaborator.id}>
      ...
      <Group mt={"300px"}>
        ...
        <Button onClick={saveNewCollaborators}>
          Save
        </Button>
      </Group>
    </Container>)
